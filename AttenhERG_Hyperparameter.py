...
#     test_prc = auc(recall, precision)
    ...
    return test_roc, test_prc, test_precision, test_recall, test_loss, test_bac, test_mcc, test_acc, test_f1


""" Hyperparameterization search"""
...
# The size of the hidden units can influence both the model's complexity and resource requirements
# So we initially fixed the dropout and L2 regularization parameters to determine the appropriate number of hidden units.
# The hidden units selection range was [50, 100, 150, 200, 250, 300], you can select one to change the value of fingerprint_dim
fingerprint_dim = 50

# L2 regularization and dropout are effective strategies to prevent overfitting. 
# We separately optimized the L2 rate and dropout rate.
# The dropout selection range was [0.1, 0.3,0.5], you can select one to change the value of p_dropout
p_dropout = 0.1
# The L2 rate selection range was [10^{-3.5}, 10^{-4},10^{-4.5}], you can select one to change the value of weight_decay
weight_decay = 3.5
...
with open('./logs/' + logs_names+ '_train_detail' + '.logs','w' ) as wpklf: 
    hy_data = [start_time,random_seed,batch_size,p_dropout,fingerprint_dim,radius,T,weight_decay,learning_rate]
    # wpklf.write('\t'.join([str(hy) for hy in hy_data]) + '\n')
    for epoch in range(epochs):    
        train_roc, train_prc, train_precision, train_recall, train_loss, _, _, _, _ = eval(model, train_df, smilesList, feature_dicts)
        valid_roc, valid_prc, valid_precision, valid_recall, valid_loss, _, _, _, _ = eval(model, valid_df, smilesList_v,feature_dicts_v)
    ...
    #     tensorboard.add_scalars('Losses',{'train_losses':train_loss,'valid_losses':valid_loss},epoch)
        ...
        if valid_roc_mean > best_param["valid_roc"]:
            ...
            if valid_roc_mean > 0.80:
                 torch.save(model, 'saved_models/model_'+ logs_names +'_'+str(epoch)+'.pt')             
        if valid_loss < best_param["valid_loss"]:
            best_param["loss_epoch"] = epoch
            best_param["valid_loss"] = valid_loss

        print("EPOCH:\t"+str(epoch)+'\t'\
            +"train_roc"+":"+str(train_roc[0])+'\t'\
            +"valid_roc"+":"+str(valid_roc[0])+'\t'\
            +"train_loss"+":"+str(train_loss)+'\t'\
            +"valid_loss"+":"+str(valid_loss)+'\t'\
    #         +"train_roc_mean"+":"+str(train_roc_mean)+'\n'\
    #         +"valid_roc_mean"+":"+str(valid_roc_mean)+'\n'\
            )
        logs_data = [str(logs_data_value) for logs_data_value in [epoch, train_roc[0], valid_roc[0], train_loss, valid_loss]]
        wpklf.write('\t'.join(logs_data) + '\n')
        if (epoch - best_param["roc_epoch"] >20) and (epoch - best_param["loss_epoch"] >30):        
            break
...
with open('./results/' +  'model_'+ logs_names +'_'+str(best_param["roc_epoch"])+'.txt', 'w') as wpklf:
    for test_type in test_type_list:
        ...
        test_roc, test_prc, test_precision, test_recall, _, test_bac, test_mcc, test_acc, test_f1 = eval(best_model, test_df, smilesList_t,feature_dicts_t)
        re = [test_type]+[str(round(i[0],3)) for i in [test_acc, test_mcc, test_bac, test_f1, test_roc, test_prc, test_precision, test_recall]]
        wpklf.write('\t'.join(re) + '\n')
